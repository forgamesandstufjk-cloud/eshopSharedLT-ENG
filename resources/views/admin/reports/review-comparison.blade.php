<x-app-layout>
    ...
    src="{{ $listing->photos->isNotEmpty()
        ? Storage::disk('photos')->url($listing->photos->first()->failo_url)
                ...
                @if($listing->photos->count() > 1)
                    <div class="flex gap-2 sm:gap-3 overflow-x-auto">
                        @foreach($listing->photos as $photo)
                            <img
    src="{{ Storage::disk('photos')->url($photo->failo_url) }}"
        ...
        <div class="space-y-4">
            @forelse($listingReviews as $review)
                <div
                    ...
                    style="
                        background-color: {{ $review->id === $selectedReview->id ? 'rgb(207, 174, 134)' : 'rgb(215, 183, 142)' }};
                        border-color: #836354;
                    "
                >
